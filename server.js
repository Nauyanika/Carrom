...
setInterval(async() => {
  try {
    let sql = "DELETE FROM   winner_name";
   
    const userss = await conn.query(sql);
    if (userss) {
...
//}, 24*60*60*1000); // for ik hour
}, 12*60*60*1000); // for ik hour
...
orderId:mainOrderId*9023,
  ...
  app.post("/uploadImageAvatar", upload.single("avatarimage"), async (req, res) => {
    console.log(req.file);
    console.log(req.body.userid);
    try {
      const existingImageQuery = "SELECT * FROM avatar_image WHERE userid = ?";
      const existingImageResult = await conn.query(existingImageQuery, req.body.userid);
      let statusCode = 200;
      let message = "";
      let sql = "";
      let formData1 = {};
      ...
      if (existingImageResult.length > 0) {
        ...
        sql = "UPDATE avatar_image SET avatarimage = ? WHERE userid = ?";
        formData1 = [req.file.filename, req.body.userid];
      } else {
        // If no existing image, insert a new one
        sql = "INSERT INTO avatar_image SET ?";
        formData1 = {
      ...
      const updateResult = await conn.query(sql, formData1);
      if (updateResult) {
  ...
  //   console.log(req.body.userid);
  //   try {
  //     let sql = "INSERT INTO  avatar_image SET ?";
  //     let formData1 = {
  //       avatarimage: req.file.filename,
  //       userid: req.body.userid,
  //     };
  ...
  // app.post('/getfetchImage', async (req, res) => {
  //   let message = null;
  //   let statusCode = 400;
  //   var data = {};
  ...
  //   try {
  //     let sql = `SELECT * FROM  avatar_image where userid=?`;
  ...
  //       var data1 = {};
  //        var image = {};
  //       image.userid = agent[0].userid;
  //       image.avatarimage = agent[0].avatarimage;
  //       data1.image = image;
  //       data = data1;
  //     } else {
  //       statusCode = 404;
  ...
  app.post('/getfetchImage', async (req, res) => {
    ...
    try {
      const sql = `SELECT * FROM avatar_image WHERE userid = ?`;
      const agent = await conn.query(sql, userid);
      ...
      if (agent.length > 0) {
        ...
        const existingAvatarImage = agent[0].avatarimage;
        const additionalString = "http://3.87.42.162/backend/images/";
        image.avatarimage = additionalString + existingAvatarImage;
